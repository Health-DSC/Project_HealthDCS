...
def classify(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            obj = Resonancia()
            obj = Resonancia.objects.create(
                                 title = img.name,
                                 img = img
                                 )
            obj.save()
            img = cv2.imread("Resultados/images/"+img.name, cv2.IMREAD_GRAYSCALE)
    ...
    if type(img)  == np.ndarray:
        ...
        types[d["yhat"]]
        obj.descripcion = types[d["yhat"]]
        obj.save()
        return JsonResponse(data=d)
    else:
        return JsonResponse(data={"Error": "Por favor, cargue una imagen correcta e intente de nuevo"})